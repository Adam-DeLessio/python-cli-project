...
def on_load():
	...
	if home == 'a':
		...
		# new_phone = ''.join(str(f"({format_phone[0:3]}) {format_phone[3:6]} {format_phone[6:]}"))
		...
		if view == 'va':
			...
			for c in view_contacts:
				print(c.fname, c.lname, c.email, c.phone)
			on_load()
		elif view == 's':
		...
		for c in search_update:
			if update_contact == c.fname:
				...
				update_field = input("Which field do you want to update? fname/lname/email/phone: ")
				if update_field == 'fname':
					...
					new_update.phone == int(input("New phone number: "))
					new_update.save()
			else:
				print("Contact does not exist.")
			on_load()
